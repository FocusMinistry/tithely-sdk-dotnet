...
using Tithely.Api.Exceptions;
using System.IO;
using Tithely.Api.Model;
using Tithely.Api.QueryObject;
...
namespace Tithely.Api {
    public abstract class BaseApiSet<T> where T : new() {
        ...
        #region Constructor
        protected BaseApiSet(TithelyOptions options, ContentType contentType = ContentType.JSON) {
            _options = options;
            _baseUrl = options.ApiUrl;
            _contentType = contentType;
        }
        #endregion Constructor

        #region Actions
        public virtual ITithelyRestResponse<T> List() {
            ...
            return item.ToTithelyResponse();
        }

        public virtual ITithelyRestResponse<T> List(string parentID) {
        ...
        public ITithelyRestResponse<List<S>> ListBySuffixUrl<S>(string url) where S : new() {
        ...
        public virtual ITithelyRestResponse<T> Get(string id) {
            ...
            var item = ExecuteCustomRequest<T>(request);
        ...
        public virtual ITithelyRestResponse<T> Get(string parentID, string id) {
        ...
        public virtual ITithelyRestResponse<T> GetByUrl(string url) {
        ...
        public virtual ITithelyRestResponse<S> GetBySuffixUrl<S>(string url) where S : new() {
            var request = CreateRestRequest(Method.GET, url);
        ...
        public virtual ITithelyRestResponse<T> GetBySuffixUrl(string url) {
            var request = CreateRestRequest(Method.GET, url);
            var item = ExecuteCustomRequest<T>(request);
        ...
        public virtual ITithelyRestResponse<S> Search<S>(BaseQO qo) where S : new() {
        ...
        private HttpClient CreateClient() {
            ...
            httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", CreateAuthValue());
            return httpClient;
        }
        ...
        public virtual IRestResponse<T> Post(string url) {
        ...
        public virtual ITithelyRestResponse<T> Create(byte[] stream, string url = "", string fileParamaterName = "stream", string fileName = "", string fileType = "") {
        ...
        public virtual ITithelyRestResponse<S> Create<S>(S entity, string url = "") where S : new() {
            var targetUrl = string.Empty;
        ...
        public virtual ITithelyRestResponse<T> Create(T entity, string url = "") {
            var targetUrl = string.Empty;
        ...
        public virtual ITithelyRestResponse<T> Create(string url = "") {
            var targetUrl = string.Empty;
        ...
        public virtual ITithelyRestResponse<T> Update(T entity, string id) {
        ...
        public virtual ITithelyRestResponse<T> Update(T entity, string id, out string requestXml) {
            if (string.IsNullOrWhiteSpace(EditUrl)) {
                throw new NotImplementedException("The property EditUrl has no value on the ApiSet.");
            }
        ...
        //public virtual ITithelyRestResponse Delete(string id) {
        //    if (string.IsNullOrWhiteSpace(EditUrl)) {
        //        throw new NotImplementedException("The property EditUrl has no value on the ApiSet.");
        //    }
        ...
        protected IRestResponse<T> ExecuteListRequest(IRestRequest request) {
            var client = new RestSharp.RestClient(_baseUrl);
            client.FollowRedirects = false;
            var response = client.Execute<T>(request);
            _parameters = new Dictionary<string, string>();
        ...
        public RestRequest CreateRestRequest(Method method, string url, string contentType = null) {
            ...
            request.AddHeader("Authorization", $"Basic {CreateAuthValue()}");
        ...
        private string CreateAuthValue() {
            var plainTextBytes = System.Text.Encoding.UTF8.GetBytes(_options.PublicKey + ":" + _options.PrivateKey);
